...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql = "UPDATE `news` SET `news_name`= '$news_name', `news_detail`= '$news_detail', `news_content`= '$news_content' $path
    ...
    <div class="container">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
            <input type="hidden" id="news_id" name="news_id" value="<?php echo $newsId; ?>" />

            <label for="news_name" class="form-label">
                <h4> News headlines </h4>
            </label>
            <input type="text" class="form-control mb-3" id="news_name" name="news_name" required value="<?php echo $news_name; ?>" />

            <label for="path" class="form-label">
                <h4> Image headlines </h4>
            </label>
            <br />
            <img style="width: 250px;" src="../../../<?php echo $path; ?>" />
            <input type="file" class="form-control mb-3 mt-3" id="path" name="uploadfile" />

            <label for="news_detail" class="form-label">
                <h4> Details </h4>
            </label>
            <textarea class="form-control mb-3" rows="5" id="news_detail" name="news_detail" required><?php echo $news_detail; ?></textarea>
            <label for="news_content" class="form-label">
                <h4> News content </h4>
            </label>
            <textarea name="news_content" class="form-control" id="news_content"><?php echo $news_content; ?></textarea>
            ...
            <button type="submit" class="mt-4 mb-4 btn btn-success btn-lg submitbtn"> Edit </button>
        </form>
    </div>
    <?php
        $conn->close();
