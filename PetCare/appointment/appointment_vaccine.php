...
<body>
                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    xhr2.onreadystatechange = function() {
                        if (this.readyState == 4 && this.status == 200) {
                            ...
                            for (let x in myObj) {
                                if (firstVaccine == 0)
                                    firstVaccine = myObj[x]["vaccine_id"];
                                vaccinePrice += "<option value='" + myObj[x]["vaccine_id"] + "'>" + myObj[x]["vaccine_name"] + "</option>";
                            }
                            console.log(vaccinePrice)
                            var vaccinePriceElement = document.getElementById('vaccine_id');
                            if (vaccinePriceElement) {
                                // If the element exists, set its innerHTML
                                vaccinePriceElement.innerHTML = vaccinePrice;
                                let priceVaccine = 0;
                                for (let x in jsonVaccine) {
                                    if (jsonVaccine[x]["vaccine_id"] == firstVaccine)
                                        priceVaccine = jsonVaccine[x]["vaccine_price"];
                                }
                                priceEle = document.getElementById("price");
                                console.log(priceVaccine);
                                if (priceEle) {
                                    // If the element exists, set its innerHTML
                                    priceEle.value = priceVaccine;;
                                } else {
                                    console.error("Element with ID 'vaccine_id' not found.");
                                }
                            } else {
                                console.error("Element with ID 'vaccine_id' not found.");
                            }
                    ...
                    xhr2.open("GET", "pet_info_vaccine.php?petId=" + petId, true);
                    xhr2.send();
    ...
    <div class="mt-4 container">
        <h1 class="mb-4 display-4"> Appointment - Vaccination </h1>
        <p class="mb-5"> สวัสดี, คุณ <?php echo $_SESSION["fname_owner"] . " " . $_SESSION["lname_owner"]; ?> </p>


        <form method="POST" action="confirm_vaccine.php">
            <div class="row">
                <div class="col- col-xl-9">
                    ...
                    <div class="mt-4 row">
                        <div class="col-6">
                            <label for="vaccine_id" class="mb-2">
                                <h4> Choose vaccine </h4>
                                <script>
                                    document.addEventListener('DOMContentLoaded', function() {
                                        var vaccineIdSelect = document.getElementById('vaccine_id');

                                        vaccineIdSelect.addEventListener("change", function() {
                                            var vaccineId = vaccineIdSelect.value;
                                            let priceVaccine = 0;
                                            for (let x in jsonVaccine) {
                                                if (jsonVaccine[x]["vaccine_id"] == vaccineId)
                                                    priceVaccine = jsonVaccine[x]["vaccine_price"];
                                            }
                                            priceEle = document.getElementById("price");
                                            console.log(priceVaccine);
                                ...
                                <select name="vaccine_id" id="vaccine_id" class="mb-4 form-select">
